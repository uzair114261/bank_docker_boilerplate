...
from bank_management_system.accounts.models import Account
...
class AccountAPIView(APIView):
    ...
    def patch(self, request):
        ...
        account = Account.objects.filter(id=account_id).select_related("bank", "user").first()

        if account and account.user == request.user:
            serializer = AccountSerializer(account, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response({"data": "Account balance updated"}, status=status.HTTP_200_OK)
            else:
    ...
    def delete(self, request):
        ...
        account = Account.objects.filter(id=account_id).select_related("user", "bank")
        if account:
            account.delete()
            return Response({"data": "deletion successful"}, status=status.HTTP_204_NO_CONTENT)
        else:
